...
24. **Pulizia e Archiviazione Automatica dei Risultati:**
    - **Richiesta:** Evitare la sovrascrittura distruttiva e lo sporcamento delle directory tra un ciclo di training/inference e l'altro, mantenendo però sempre un salvataggio di sicurezza.
    - **Soluzione:** Modificato `train_model.py` in modo che all'avvio svuoti la cartella `dino_kvasir_model/backup` e ci sposti automaticamente all'interno tutte le vecchie cartelle `fold_n` prima di iniziare un nuovo ciclo di addestramento. In `run_model.py` è stata introdotta una logica di *archiviazione*: prima di eseguire una nuova valutazione, eventuali cartelle `fold_` ed `ensemble` presenti in `results/` vengono raggruppate in una nuova sottocartella nominata con il timestamp esatto del momento (es. `results/20260728_143000/`), preservando così lo storico degli esperimenti.
