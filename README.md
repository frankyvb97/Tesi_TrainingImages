...
24. **Pulizia e Archiviazione Automatica dei Risultati:**
    ...
    - **Soluzione:** Modificato `train_model.py` in modo che all'avvio svuoti la cartella `dino_kvasir_model/backup` e ci sposti automaticamente all'interno tutte le vecchie cartelle `fold_n` prima di iniziare un nuovo ciclo di addestramento. In `run_model.py` è stata introdotta una logica di *archiviazione diretta*: le matrici di confusione e le metriche di un run vengono ora salvate direttamente all'interno di una cartella generata sul momento col timestamp esatto (es. `results/20260728_154100/`), facendo combaciare perfettamente data e ora e azzerando il rischio di sovrascritture.
